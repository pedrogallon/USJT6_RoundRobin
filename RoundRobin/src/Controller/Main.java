...
public class Main {
	...
	static List<Processo> filaDisco = new ArrayList<Processo>();
	static int momentoExecucao = 0;
	static int memoriaAtual = 0;
	static int memoria = 100;
...
//		}
		...
		while (!fila.isEmpty()) {
			...
			placeholderVerificarSeInsere(pegarProximosProcessos());
...
//			System.out.println("Processo " + processo.getNome() + " pulado. \n");
	...
	public static void placeholderVerificarSeInsere(Processo[] processos) {
		for (Processo p : processos)
			if (p.getDuracao() + getMemoria() <= memoria) {
				fila.add(p);
				filaDisco.remove(0);
			}
	}

	public static Processo[] pegarProximosProcessos() {
		// n'ao tirar da fila ainda
		Processo[] processos = new Processo[10];
		for (int i = 0; i < filaDisco.size(); i++) {
			if (filaDisco.get(i).getChegada() <= momentoExecucao) {
				processos[i] = filaDisco.get(i);
			}
		}
		return processos;
	}
